...
export default function HomePage () {
  ...
  const MovieCard = ({ movie }) => {
    const posterUrl =
      movie.Poster !== 'N/A'
        ? movie.Poster
        : 'https://placehold.co/400x600/5D688A/FFDBB6?text=No+Poster+Available'
    return (
      <Link
  ...
  return (
    <div className='body-container'>
      ...
      <Link to='/suggest' className='movie-button' style={{textDecoration:"none", position:"relative", left:"70%"}}>
      ...
      <main className='main-section'>
        <div className='user-div'>
          <input
            ...
            onKeyDown={e => {
              if (e.key === 'Enter') handleSearch(1, e.target.value)
            }}
      ...
      {showLoadMore && (
        <button id='pages-button' onClick={handlePage} className='movie-button'>
          {searchMoreLoading ? 'Loading...' : 'Show More'}
        </button>
      )}
    </div>
  )
